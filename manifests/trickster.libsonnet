...
{
  cache(name, namespace, host, port = 9090, listen_port = 9090): {
    local this = self,

    image: 'tricksterio/trickster:0.1.6',
    config: |||
      [main]
    ...
    ||| % {host: host, port: port, listen_port: listen_port},

    configmap: ok.ConfigMap('%s-config' % name, namespace, app=name) {
      data: {
        'trickster.conf': this.config
      },
    },
  },
}
